...
model = pickle.load(
    open(model_path, 'rb'))
model = pickle.load(open('savedmodels/classifier.pkl','rb'))
...
def fertilizer_predict(request):
   ...
   if request.method=='POST':
     ...
     input = np.array([[temp,humi,mois,soil,crop,nitro,pota,phosp]])

     res = ferti.classes_[model.predict(input)]

     return render(request,'fertilizer-result.html',{'recommendation':res})    
